...
import { getTickets as getStoredTickets } from "../utils/ticketStore";

function normalizeTicketForOverview(t) {
  const category =
    t?.category ||
    t?.serviceCategory ||
    t?.issueCategory ||
  ...
  const statusUpper = String(
    t?.status || t?.ticketStatus || "NEW"
  ).toUpperCase();

  const routingUpper = String(t?.routingStatus || "").toUpperCase();
  const stageUpper = String(t?.workflowStage || "").toUpperCase();

  const normalizedStatus =
    routingUpper === "REJECTED" ||
    stageUpper === "REJECTED" ||
    stageUpper === "REJECTED_BY_SUPERVISOR"
      ? "REJECTED"
      : statusUpper === "DELETE" || statusUpper === "DELETED"
      ? "DELETE"
      : statusUpper;

  return {
    ticketNumber: t?.ticketNumber || t?.ticketId || t?.id || "—",
...
export default function LoginPage() {
  ...
  const [overviewLoadedFrom, setOverviewLoadedFrom] = useState("LIVE");
  ...
  const loadOverviewTickets = useCallback(async () => {
    try {
      const apiTickets = await fetchTickets();
      const normalized = (Array.isArray(apiTickets) ? apiTickets : []).map(
        normalizeTicketForOverview
      );
      setOverviewTickets(normalized);
      setOverviewLoadedFrom("LIVE");
    } catch (e) {
      const fallback = (getStoredTickets() || []).map(normalizeTicketForOverview);
      setOverviewTickets(fallback);
      setOverviewLoadedFrom("LOCAL");
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const refresh = () => loadOverviewTickets();
    ...
    () =>
      ({
        EN: {
          ...
          overviewSub:
            "Snapshot of current tickets loaded in the system.",
          invalidPassword: "Invalid password.",
          loginHelp:
            "Use the assigned demo password for the selected role account.",
          notesTitle: "Notes",
          note1:
            "This overview now reflects the current loaded ticket dataset instead of static mock data.",
          note2:
          ...
          french: "Français (FR)",
          modeBanner:
            "Prototype mode • Current dataset • Some actions are simulated",
          footerAbout:
          ...
          loadedFromLive: "Data source: Live/backend",
          loadedFromLocal: "Data source: Local fallback",
        },
        FR: {
          navHome: "ACCUEIL",
          ...
          overviewSub:
            "Instantané des billets actuellement chargés dans le système.",
          invalidPassword: "Mot de passe invalide.",
          loginHelp:
            "Utilisez le mot de passe de démonstration assigné au compte sélectionné.",
          notesTitle: "Notes",
          note1:
            "Cet aperçu reflète maintenant le jeu de billets actuel au lieu de données mock statiques.",
          note2:
          ...
          french: "Français (FR)",
          modeBanner:
            "Mode prototype • Jeu actuel • Certaines actions sont simulées",
          footerAbout:
          ...
          loadedFromLive: "Source des données : backend",
          loadedFromLocal: "Source des données : secours local",
        },
      }[lang]),
    [lang]
  );
  ...
  const users = [
    { name: SUPERVISOR?.name || "Nagavalli", role: "SUPERVISOR", password: "Naga@311" },
  ];
  ...
  const overview = useMemo(() => {
    const all = Array.isArray(overviewTickets) ? overviewTickets : [];
    ...
    const recentTickets = [...all]
      ...
      .map((t) => ({
        ...
        status: String(t?.status || "NEW").toUpperCase(),
      }));

    return {
      total,
      openCount: all.filter((t) => {
      ...
      workflow: [
        ...
        { label: "ESCALATED", value: byStatus.ESCALATED || 0 },
            ...
            <div className="card lpOverviewCard">
              <div className="lpOverviewHead">
                <div>
                  ...
                  <div className="lpMuted" style={{ marginTop: 6, fontSize: 12 }}>
                    {overviewLoadedFrom === "LIVE"
                      ? copy.loadedFromLive
                      : copy.loadedFromLocal}
                  </div>
                </div>
