...
import {
  ...
  markFalseReport as markFalseReportApi,
  unmarkFalseReport as unmarkFalseReportApi,
} from "../api/tickets";
...
function normalizeTicketForDraft(ticket) {
  ...
  const departmentStatus =
    ticket.departmentStatus ||
    ticket.department_status ||
    "NOT_STARTED";
...
export default function TicketDetailsPage() {
  ...
  const [falseReporting, setFalseReporting] = useState(false);
  ...
  const isEscalated = normalizedStatus === TICKET_STATUSES.ESCALATED;
  const isDeleted = normalizedStatus === TICKET_STATUSES.DELETE;
  const isTerminal = rejected || isResolved || isEscalated || isDeleted;
  const pureVoiceBot = isPureVoiceBotTicketSafe(ticket);
  ...
  const createdByName =
    ...
    (upper(createdByType) === "VOICE_BOT" ? "INSIGHT VoiceBot" : "-");
  const isVoiceBotTicket = upper(createdByType) === "VOICE_BOT";

  const handledByRole =
    ticket?.handledByRole ||
    (pureVoiceBot ? "VOICE_BOT" : isVoiceBotTicket ? "VOICE_BOT" : "OPERATOR");
  const handledByName =
    ticket?.handledByName ||
    (pureVoiceBot ? "INSIGHT VoiceBot" : createdByName || "-");
  const handledByType =
    ticket?.handledByType ||
    (pureVoiceBot ? "VOICE_BOT" : isVoiceBotTicket ? "VOICE_BOT" : "OPERATOR");

  const approvedTimestamp = ticket?.approvedAt || (approved ? ticket?.updatedAt : "");
  const confidenceReasons = buildConfidenceReasons(ticket);
  const showConfidenceReasons =
    isVoiceBotTicket &&
    (upper(ticket?.confidence) !== "HIGH" ||
      upper(ticket?.confidenceAlert) === "LOW_CONFIDENCE" ||
  ...
  async function doSave() {
    ...
    try {
      ...
      const payload = {
        ...
        departmentStatus: draft.departmentStatus || "NOT_STARTED",
        notes: draft.notes || "",
      };
  ...
  async function doApprove() {
    ...
    try {
      ...
      const departmentStatus = draft?.departmentStatus || "ASSIGNED";
      ...
      if (
        ...
        departmentStatus !== (ticket.departmentStatus || "NOT_STARTED")
      ) {
      ...
      const patched = await updateTicketApi(apiId, {
        assignedDepartment: department,
        department,
        departmentStatus,
      });
  ...
  async function doEscalate() {
    ...
    try {
      const apiId = requireBackendId(ticket);
      const updated = await escalateTicketApi(apiId);
      ...
      const mergedUpdated = mergeTicketPreservingEvidence(ticket, {
        ...
        handledByRole: "SUPERVISOR",
        departmentStatus:
          draft?.departmentStatus || ticket?.departmentStatus || "COMPLETED",
      });
  ...
  async function doFalseReport() {
    if (!ticket) return;
    setFalseReporting(true);
    try {
      const apiId = requireBackendId(ticket);
      const isCurrent = ticket.isFalseReport;
      const updated = isCurrent
        ? await unmarkFalseReportApi(apiId)
        : await markFalseReportApi(apiId);
      setTicket(mergeTicketPreservingEvidence(ticket, updated));
      notifyTicketsChanged();
      toast({
        type: isCurrent ? "info" : "warning",
        title: isCurrent ? "False report cleared" : "Marked as false report",
        msg: isCurrent
          ? "False report flag removed."
          : "Penalty recorded for this phone number.",
      });
    } catch (error) {
      toast({ type: "error", title: "Failed", msg: "Could not update false report status." });
    } finally {
      setFalseReporting(false);
    }
  }
  ...
  const callHandledByLabel = (() => {
    ...
    if (upper(createdByType) === "VOICE_BOT" && role === "OPERATOR") {
      return `Voice Bot → Operator (${handledByName})`;
    }
    if (upper(createdByType) === "VOICE_BOT" && role === "SUPERVISOR") {
      return `Voice Bot → Supervisor (${handledByName})`;
    }
  ...
  const busy = approving || rejecting || resolving || escalating || falseReporting;
  const isFalseReport = ticket.isFalseReport ?? false;
      ...
      <div className="ticketStickyHeader">
        ...
        <div className="ticketHeaderRow">
          ...
          <div className="ticketHeaderActions">
            ...
            {canStaffAct && !isDeleted ? (
              <button
                className="btn"
                type="button"
                disabled={busy}
                onClick={doFalseReport}
                style={{
                  fontSize: 12,
                  background: isFalseReport ? "#fef2f2" : undefined,
                  color:      isFalseReport ? "#dc2626" : "#6b7280",
                  border:     isFalseReport ? "1.5px solid #fca5a5" : undefined,
                }}
                title={isFalseReport ? "Click to undo false report" : "Mark as a false / malicious report"}
              >
                {falseReporting ? "…" : isFalseReport ? "⚑ False Report" : "⚐ False Report"}
              </button>
            ) : null}
          ...
          <div className="card ticketCard">
            ...
            >
              ...
              <span className="metaPill metaPillMuted">
                Dept status: {(ticket.departmentStatus || "NOT_STARTED").replaceAll("_", " ")}
              </span>
            ...
            <div style={{ marginTop: 14, display: "grid", gap: 14 }}>
                ...
                <div className="ticketSummaryGrid">
                  <Field label="Created">
                    <div className="lpMuted" style={{ fontWeight: 900 }}>
                      <TimestampStack value={ticket.createdAt} />
                    </div>
                  </Field>
                  {approved ? (
                    <Field label="Approved">
                      <div className="lpMuted" style={{ fontWeight: 900 }}>
                        <TimestampStack value={approvedTimestamp} />
                      </div>
                    </Field>
                  ) : null}
                  <Field label="Status">
                  ...
                  <Field label="Department status">
                    <div className="lpMuted" style={{ fontWeight: 900 }}>
                      {(ticket.departmentStatus || "NOT_STARTED").replaceAll("_", " ")}
                    </div>
                  </Field>
                  ...
                  <Field label="Created by">
                    <div className="lpMuted" style={{ fontWeight: 900 }}>
                      {ticket.createdByName || (isVoiceBotTicket ? "INSIGHT VoiceBot" : "-")}
                    </div>
                  </Field>
                  <Field label="Handled by">
                    <div className="lpMuted" style={{ fontWeight: 900 }}>
                      {ticket.handledByName || (pureVoiceBot ? "INSIGHT VoiceBot" : "-")}
                    </div>
                  </Field>
                </div>
                ...
                <div style={{ background: "#f3f4f6", padding: 12, borderRadius: 12 }}>
                  ...
                  {evidenceOpen ? (
                    <div style={{ marginTop: 10, display: "grid", gap: 10 }}>
                      ...
                      >
                        {(() => {
                          const raw = ticket.transcript || ticket.description || "-";
                          if (!raw || raw === "-") return "-";
                          const lines = raw.split("\n");
                          const clean = lines
                            .filter(
                              (l) =>
                                !l.match(/^(Session ID|Ticket|Timestamp|={10})/i) &&
                                l.trim()
                            )
                            .map((l) => l.trim())
                            .filter((l) => l.length > 0)
              ...
              <Field label="Department status">
                <select
                  value={draft?.departmentStatus || "NOT_STARTED"}
                  disabled={!isEditing || isTerminal}
                  onChange={(e) =>
                    setDraft((current) => ({
                      ...current,
