...
function normalizeCategory(value = "") {
  ...
  const map = {
    ...
    "Litter in a playground, park or trail": "Litter",
    "Property standards complaint": "Other",
...
function normalizeChannel(value = "", fallback = "WEB") {
  ...
  if (raw === "PHONE" || raw === "CALL" || raw === "VOICE") return "PHONE";
  if (raw === "EMAIL" || raw === "E-MAIL") return "EMAIL";
  if (raw === "IN_PERSON" || raw === "IN-PERSON" || raw === "IN PERSON") {
    return "IN_PERSON";
  }
...
function pickNextOperatorName() {
  ...
  if (names.length === 0) return "Nagavalli";
...
export default function PublicRequestPage() {
  ...
  const copy =
    lang === "FR"
      ? {
          ...
          submitFailedTitle: "Erreur de soumission",
          submitFailedMsg:
            "Impossible de soumettre la demande pour le moment. Veuillez réessayer.",
          required:
            "Veuillez remplir le nom, le téléphone et la description.",
          requiredLocation:
            "Veuillez remplir le nom, le téléphone, le lieu et la description.",
      ...
      : {
          ...
          requiredLocation:
            "Please fill in name, phone, location, and description.",
          validationTitle: "Validation",
  ...
  async function submitRequest(e) {
    ...
    if (!name.trim() || !phone.trim() || !locationText.trim() || !description.trim()) {
      toast.error(
        copy.requiredLocation || "Please fill in name, phone, location, and description.",
    ...
    const resolvedCategory = initialCategory || "Other";
    const resolvedDepartment =
      routedDepartment || inferDepartmentFromCategory(resolvedCategory) || "";
    const assignedOperator = SUPERVISOR?.name || pickNextOperatorName();
    ...
    const payload = {
      ...
      channel: normalizeChannel("WEB"),
      tone: normalizeTone("UNKNOWN"),
      callerTone: normalizeTone("UNKNOWN"),
      ...
      handledByName: assignedOperator,
    ...
    try {
      ...
      const createdTicketNumber = created?.ticketNumber || created?.ticketId || "";

      window.dispatchEvent(new Event("tickets-changed"));

      toast.success(`${copy.submittedMsg} ${createdTicketNumber}.`, {
      ...
      toast.error(err?.message || copy.submitFailedMsg, {
          ...
          {(initialCategory || routedDepartment) && (
            <div className="lpInfo" style={{ marginBottom: 18 }}>
              ...
              <div style={{ marginBottom: 4 }}>
                <strong>{copy.category}:</strong> {initialCategory || "Other"}
              </div>
              <div style={{ marginBottom: 4 }}>
          ...
          <div className="lpGrid">
            <div className="lpCard">
              ...
              <form onSubmit={submitRequest} style={{ marginTop: 16 }}>
                ...
                <div className="lpFormGroup">
                  <label htmlFor="public-location">{copy.location}</label>
                  <input
                    id="public-location"
                ...
                >
                  <button type="submit" className="lpButton" disabled={isSubmitting}>
