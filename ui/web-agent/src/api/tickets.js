...
export function mapBackendTicketToUi(ticket) {
  ...
  const rawChannel = ticket.channel ?? "";
  const rawCreatedByType = ticket.created_by_type ?? ticket.createdByType ?? "";
  const channel =
    String(rawCreatedByType).toUpperCase() === "VOICE_BOT" ? "VOICE" : rawChannel;
...
export async function approveTicket(ticketId) {
  ...
  const data = await apiFetch(`/tickets/${encodeURIComponent(ticketId)}/approve`, { method: "POST" });
  return data ? mapBackendTicketToUi(data) : null;
}
