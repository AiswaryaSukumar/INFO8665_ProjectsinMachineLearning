...
export function decideHandoffTarget({
  ...
  if (!operatorName || String(operatorName).trim().toLowerCase() === String(supervisorName).trim().toLowerCase()) {
    return {
      handledByType: "VOICE_BOT_TO_HUMAN",
      handledByRole: "SUPERVISOR",
      handledByName: supervisorName,
      escalation: {
        escalatedToRole: "SUPERVISOR",
        escalatedToName: supervisorName,
        escalationReason: "No active operator available",
      },
    };
  }

  // Normal transfer: to an active operator.
  return {
    handledByType: "VOICE_BOT_TO_HUMAN",
