...
class EntityExtractor:
    ...
    BAD_NAME_TOKENS = {
        ...
        "pothole", "issue", "problem", "street", "road",
    ...
    def _is_name_candidate_valid(self, candidate: str) -> bool:
        ...
        'calling to', 'to report a pothole', etc. as caller names.
        ...
        return (bad_count / len(tokens)) < 0.5
    ...
    def extract_caller_name(self, text: str) -> Tuple[Optional[str], float]:
        ...
        if not person_entities:
            ...
            for pattern in name_patterns:
                ...
                if match:
                    ...
                    # Heuristic filter to avoid things like "Calling To", "To Report"
        ...
        # Use the first PERSON entity detected by spaCy
    ...
    def extract_location(self, text: str) -> Tuple[Optional[str], float]:
        ...
        if not location_entities:
            ...
            street_patterns = [
                    # Prefixed: "at/on/location is/located at <address>"
                    r"(?:location\s+is|located\s+at|on|at)\s+((?:\d+\s+)?[a-zA-Z][a-zA-Z\s]+?\s+(?:Street|Avenue|Road|Boulevard|Drive|Lane|Court|Place|Way|Blvd|Ave|Rd|St)(?:\s+(?:North|South|East|West|NE|NW|SE|SW))?)",
                    # Bare address: "334 King Street North"
                    r"((?:\d+\s+)?[a-zA-Z][a-zA-Z\s]+?\s+(?:Street|Avenue|Road|Boulevard|Drive|Lane|Court|Place|Way|Blvd|Ave|Rd|St)(?:\s+(?:North|South|East|West|NE|NW|SE|SW))?)",
            ]
