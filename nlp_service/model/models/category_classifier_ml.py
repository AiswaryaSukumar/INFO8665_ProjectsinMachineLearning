...
from config import (
    ...
    NUM_LABELS as DEFAULT_NUM_LABELS,
    MAX_SEQ_LENGTH as DEFAULT_MAX_SEQ_LENGTH,
...
ml_logger = get_logger("insight311.ml_classifier")
...
class DistilBERTCategoryClassifier:
    ...
    def __init__(
        ...
        ml_logger.info(
            "classifier_init model=%s num_labels=%d max_length=%d device=%s",
            model_name, num_labels, max_length, self.device,
        )
    ...
    def train(
        ...
        print()
        ml_logger.info(
            "training_started experiment=%s version=%s samples=%d epochs=%d "
            "batch_size=%d lr=%s",
            EXPERIMENT_NAME, EXPERIMENT_VERSION, len(train_texts),
        ...
        ml_logger.info("training_completed loss=%.4f", train_result.training_loss)
        ...
        if eval_dataset:
            ...
            ml_logger.info(
                "evaluation_results accuracy=%.4f loss=%.4f expected_accuracy=%s",
                accuracy, eval_result['eval_loss'], EXPECTED_ACCURACY,
            )
            if accuracy < EXPECTED_ACCURACY:
                ml_logger.warning(
                    "accuracy_below_expected actual=%.4f expected=%s",
        ...
        print("=" * 70)
        ml_logger.info("model_saved output_dir=%s", output_dir)
    ...
    def load_model(self, model_dir: str = "saved_models/category_classifier"):
        ...
        ml_logger.info("model_loaded dir=%s device=%s", model_dir, self.device)
