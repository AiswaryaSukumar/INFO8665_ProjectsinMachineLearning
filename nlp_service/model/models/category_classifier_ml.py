...
# ---------------------------------------------------------------------------
# Semantic Fallback — category description index for cosine-similarity search
# Used when DistilBERT softmax confidence is below SEMANTIC_FALLBACK_THRESHOLD
# ---------------------------------------------------------------------------
CATEGORY_DESCRIPTIONS: Dict[str, str] = {
    "pothole":            "pothole road damage pavement crack hole asphalt surface broken street road repair",
    "graffiti":           "graffiti vandalism spray paint tagging wall writing defacement public property",
    "illegal_sign":       "illegal sign unauthorized signage billboard advertisement permit violation posting",
    "litter":             "litter garbage trash waste rubbish illegal dumping debris scattered on street",
    "needles":            "needles syringes drug paraphernalia sharps injection drug use on ground park",
    "parking_complaint":  "parking illegal parking violation blocking hydrant fire zone no parking zone",
    "sidewalk_snow":      "snow ice sidewalk unplowed slippery icy winter snow removal path blocked",
    "sidewalk_hazard":    "sidewalk hazard broken cracked uneven trip fall danger pedestrian walking",
    "trail_maintenance":  "trail maintenance path repair park overgrown damaged walking cycling route",
    "property_standards": "property standards building bylaw violation dilapidated unsafe structure disrepair",
    "emergency":          "emergency urgent dangerous life threatening immediate help fire accident injury",
    "other":              "other issue general complaint miscellaneous unknown service request",
}

# Trigger semantic search when DistilBERT top-class probability is below this
SEMANTIC_FALLBACK_THRESHOLD = 0.45
# Only accept semantic result if best cosine similarity exceeds this
SEMANTIC_SIMILARITY_MIN     = 0.52
...
class DistilBERTCategoryClassifier:
    ...
    def __init__(
        ...
        if auto_load:
            try:
                ...
                self.load_model("nlp_service/ml_models/saved_models/category_classifier")
            except Exception as e:
                print(f"⚠️  Could not auto-load model: {e}")
                print("   You can manually call load_model() or train a new model")
    
    def load_categories(self, categories_file: str = "nlp_service/data/models/categories.json"):
        """
    ...
    def load_model(self, model_dir: str = "saved_models/category_classifier"):
        ...
        # Build semantic index for fallback (uses already-loaded model weights)
        try:
            self._build_semantic_index()
        except Exception as e:
            print(f"⚠️  Semantic index build failed: {e}. Semantic fallback disabled.")
    ...
    def _get_mean_pooled_embedding(
        self,
        input_ids: torch.Tensor,
        attention_mask: torch.Tensor,
    ) -> torch.Tensor:
        """Mean-pool DistilBERT last hidden states over non-padding tokens."""
        base_out   = self.model.distilbert(input_ids=input_ids, attention_mask=attention_mask)
        hidden     = base_out.last_hidden_state                      # [B, seq, dim]
        mask_exp   = attention_mask.unsqueeze(-1).float()            # [B, seq, 1]
        summed     = (hidden * mask_exp).sum(dim=1)                  # [B, dim]
        counts     = mask_exp.sum(dim=1).clamp(min=1e-9)             # [B, 1]
        return summed / counts                                        # [B, dim]

    def _build_semantic_index(self) -> None:
        """
        Pre-compute L2-normalised embeddings for all CATEGORY_DESCRIPTIONS.
        Called once during load_model(); adds ~1 s at startup, zero cost at inference.
        """
        ...
        for cat, desc in CATEGORY_DESCRIPTIONS.items():
            ...
            with torch.no_grad():
                emb = self._get_mean_pooled_embedding(ids, mask).squeeze(0)  # [dim]
            embeddings.append(emb)
            categories.append(cat)

        raw_matrix = torch.stack(embeddings, dim=0)                  # [N, dim]
        norms      = raw_matrix.norm(dim=1, keepdim=True).clamp(min=1e-9)
        self._semantic_embeddings_norm = raw_matrix / norms          # unit vectors
        self._semantic_categories      = categories
        print(f"✓ Semantic fallback index built ({len(categories)} categories)")

    def _semantic_fallback(self, text: str) -> Tuple[Optional[str], float]:
        """
        Encode *text* with mean-pooled DistilBERT embeddings and return
        (best_category, cosine_similarity).  Returns (None, 0.0) when the
        semantic index is unavailable.
        """
        if not hasattr(self, "_semantic_embeddings_norm"):
            return None, 0.0
        ...
        with torch.no_grad():
            query     = self._get_mean_pooled_embedding(ids, mask)   # [1, dim]
            q_norm    = query / query.norm(dim=1, keepdim=True).clamp(min=1e-9)
            sims      = (q_norm @ self._semantic_embeddings_norm.T).squeeze(0)  # [N]
            best_idx  = sims.argmax().item()
            best_score = float(sims[best_idx].item())

        return self._semantic_categories[best_idx], round(best_score, 4)
    ...
    def predict(self, text: str) -> Dict:
        ...
        # ------------------------------------------------------------------
        # Semantic fallback: when DistilBERT is uncertain (STT noise, etc.)
        # re-rank using cosine similarity against category description index.
        # ------------------------------------------------------------------
        used_semantic = False
        if confidence < SEMANTIC_FALLBACK_THRESHOLD:
            sem_category, sem_score = self._semantic_fallback(text)
            if sem_score >= SEMANTIC_SIMILARITY_MIN and sem_category is not None:
                category      = sem_category
                # Cap semantic confidence below 1.0; slightly lower than a
                # direct DistilBERT prediction to preserve signal ordering.
                confidence    = min(0.82, sem_score)
                used_semantic = True
        ...
        return {
            ...
            'used_semantic': used_semantic,          # debug flag
        }
