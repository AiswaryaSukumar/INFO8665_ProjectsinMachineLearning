...
def initialize_stt_dirs():
    ...
    if model is None:
        try:
            ...
            print(f"Warning: STT Model could not be loaded: {e}")

model = None

def measure_ambient_noise(duration: float = 1.5,
                          sample_rate: int = 16000,
                          device: int = 1) -> float:
    """
    Record `duration` seconds of audio and return the RMS noise floor.
    Used to dynamically set silence_threshold before a call starts.
    """
    n_samples = int(sample_rate * duration)
    print(f"  🔇 Measuring ambient noise for {duration}s...")
    try:
        ...
        print(f"  ⚠️  Ambient noise measurement failed: {e}. Using default 0.03.")
        return 0.03
...
PHONETIC_MAP = {
    ...
    "litter": ["leader", "liter", "leather", "letter", "litter problem", "letter problem"],
