...
class Orchestrator:
    ...
    def process_turn(self, session_id: str, transcript: str, nlu_output: Dict[str, Any]) -> Tuple[ConversationContext, Action]:
        ...
        # ── Intercept 3: CONFIRMATION + yes → skip all entity updates, submit immediately ──
        # Must run BEFORE update_context_with_nlu so that ambient noise mixed into the
        # confirmation utterance cannot overwrite the already-confirmed slot values.
        if context.current_state == ConversationState.CONFIRMATION:
            if nlu_output.get("confirm_result") == "yes":
                ...
                self.context_manager.save_context(context)
                return context, Action(action_type="submit", text=submit_text, new_state=ConversationState.SUBMITTED, ticket_id=ticket_id)
        ...
        if context.current_state == ConversationState.CONFIRMATION:
            ...
            # --- 2. Explicit no with no detectable correction → ask what's wrong ---
