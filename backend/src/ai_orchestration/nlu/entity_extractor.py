...
class EntityExtractor:
    ...
    def extract_location(self, text: str) -> Tuple[Optional[str], float]:
        ...
        if not location_entities:
            ...
            street_patterns = [
                    r"(?:on|at)\s+((?:\d+\s+)?[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\s+(?:Street|Avenue|Road|Boulevard|Drive|Lane|Court|Place)(?:\s+(?:North|South|East|West))?)",
                    r"((?:\d+\s+)?[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\s+(?:Street|Avenue|Road|Boulevard|Drive|Lane|Court|Place)(?:\s+(?:North|South|East|West))?(?:\s+near\s+[A-Z][a-z]+)?)",
            ]
